...
if ($content -match 'campus\.local') {
    $content = $content -replace '\d+\.\d+\.\d+\.\d+\s+campus\.local', "$minikubeIp  campus.local"
    Write-Host "✅ Updated campus.local → $minikubeIp" -ForegroundColor Green
} else {
    $content += "`n$minikubeIp  campus.local"
    Write-Host "✅ Added campus.local → $minikubeIp" -ForegroundColor Green
}

# ── Staging: staging.campus.local ────────────────────────────
if ($content -match 'staging\.campus\.local') {
    $content = $content -replace '\d+\.\d+\.\d+\.\d+\s+staging\.campus\.local', "$minikubeIp  staging.campus.local"
    Write-Host "✅ Updated staging.campus.local → $minikubeIp" -ForegroundColor Green
} else {
    $content += "`n$minikubeIp  staging.campus.local"
    Write-Host "✅ Added staging.campus.local → $minikubeIp" -ForegroundColor Green
}
